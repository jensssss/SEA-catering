...
// --- DATA ---
// Using const assertions for better type inference and immutability
const PLANS = [
...
// --- TYPE DEFINITIONS ---
// Deriving types from our data constants ensures they are always in sync
...
// A checkmark icon for visual feedback
const CheckIcon = () => (
...
// A consistent header for each section of the form
...
// A clickable "chip" button for selections like meals and days
...
export default function App() {
    ...
    // Using useMemo to prevent recalculating the price on every render
    const totalPrice = useMemo(() => {
        if (!selectedPlan || selectedMeals.length === 0 || selectedDays.length === 0) {
            return 0;
        }
        // Calculation based on weeks in a month (average)
    ...
    // --- EVENT HANDLERS ---
    // Generic toggle handler for arrays of items (meals, days)
    const handleToggle = <T,>(item: T, selectedItems: T[], setSelectedItems: React.Dispatch<React.SetStateAction<T[]>>) => {
    ...
    try {
        // Use fetch to send a POST request to our new API endpoint
        ...
        // If successful, show the success message from the API
        setSubmitMessage(`Thank you, ${name}! Your subscription has been confirmed.`);

        } catch (error) {
            // If there's an error in the fetch call, show it to the user
            console.error('Submission error:', error);
            setSubmitMessage(error.message || 'Failed to submit subscription. Please try again.');
        } finally {
            // Always stop the submitting state, whether it succeeded or failed
            setIsSubmitting(false);
        }
    };
                ...
                <form onSubmit={handleSubmit} className="space-y-12">
                    ...
                    <section className="pt-8 border-t border-slate-200">
                       <div className="bg-slate-100/70 p-6 rounded-xl">
                            ...
                            {submitMessage && (
                                <p className={`mt-4 text-center text-sm font-medium ${submitMessage.startsWith('Thank') ? 'text-green-600' : 'text-red-600'}`}>
                                    {submitMessage}
                                </p>
                            )}
                       </div>
